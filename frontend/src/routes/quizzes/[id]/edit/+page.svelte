<script lang="ts">
  ...
  onMount(async () => {
    ...
    if (quizRes.data) {
      ...
      const sqRes = await api<{ question_id: string; marks: number }[]>(
        "GET",
        `/quizzes/${quizId}/questions`,
      );
      if (sqRes.data) {
        selectedQuestions = sqRes.data.map((sq) => ({
          id: sq.question_id,
          marks: sq.marks,
        }));
      }
  ...
  async function save() {
    saving = true;
    try {
      const putRes = await api("PUT", `/quizzes/${quizId}`, {
        title,
        description,
        target_type,
      ...
      if (putRes.error) {
        toast(putRes.error.message, "error");
        saving = false;
        return;
      }
      ...
      {#each filteredQuestions as q (q.id)}
        <div class="flex items-center gap-3 p-3 hover:bg-slate-50">
          ...
          <div class="flex-1 min-w-0">
            <p class="text-sm text-slate-900 line-clamp-1">
          ...
          {#if selectedIds.has(q.id)}
            <input
              type="number"
              value={selectedQuestions.find((sq) => sq.id === q.id)?.marks || 1}
              oninput={(e) =>
