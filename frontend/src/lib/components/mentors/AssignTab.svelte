<script lang="ts">
  ...
  let { year, isAdmin }: { year: string; isAdmin: boolean } = $props();
  ...
  let stats = $state<{
    unassigned: number;
    mentors: { id: string; name: string; student_count: number }[];
  } | null>(null);
  let unassignedStudents = $state<
    { id: string; name: string; sats_number: string }[]
  >([]);
  ...
  let unassignLoading = $state<Set<string>>(new Set());

  const selectedMentor = $derived(
    stats?.mentors.find((m) => m.id === selectedMentorId),
  );
  ...
  async function load() {
    ...
    loading = true;
    errorMsg = "";
    const params = new URLSearchParams({ academic_year_id: year });
    ...
    const [aRes, sRes, uRes] = await Promise.all([
      ...
      api<typeof unassignedStudents>(
        "GET",
        `/students?${params}&unassigned=true`,
      ),
    ]);
  ...
  async function assign() {
    ...
    try {
      for (const sid of selectedStudentIds) {
        const res = await api("POST", "/mentors/assignments", {
  ...
  async function unassign(id: string) {
    if (unassignLoading.has(id)) return;
    if (!confirm("Remove this student from this mentor?")) return;
    statusMsg = "";
    errorMsg = "";
    unassignLoading = new Set(unassignLoading).add(id);
    const res = await api("DELETE", `/mentors/assignments/${id}`);
    if (res.error) {
      errorMsg = res.error.message;
      toast(res.error.message, "error");
    } else {
      toast("Mentor assignment removed", "success");
      await load();
    }
    unassignLoading = new Set([...unassignLoading].filter((x) => x !== id));
  }
  ...
  function selectMentor(id: string) {
    selectedMentorId = selectedMentorId === id ? "" : id;
    selectedStudentIds = new Set();
  }
</script>
...
<div class="space-y-6">
  ...
  {#if errorMsg}
    <div
      class="flex items-center gap-2 text-sm px-4 py-2.5 rounded-lg bg-red-50 text-red-700 border border-red-200"
    >
      <AlertCircle size={16} />
      {errorMsg}
    </div>
  ...
  {:else}
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
      ...
      >
        <div
          class="w-10 h-10 rounded-lg bg-teal-50 flex items-center justify-center"
        >
          <Users size={18} class="text-teal-600" />
        </div>
        <div>
          <div class="text-2xl font-bold text-slate-900">
            {stats?.mentors.length ?? 0}
          </div>
          <div class="text-xs text-slate-500">Mentors</div>
        </div>
      </div>
      <div
        class="bg-white rounded-xl border border-slate-200 p-4 flex items-center gap-3"
      >
        ...
        >
          <GraduationCap size={18} class="text-blue-600" />
        </div>
        <div>
          <div class="text-2xl font-bold text-slate-900">{totalAssigned}</div>
          <div class="text-xs text-slate-500">Students assigned</div>
        </div>
      </div>
      <div
        class="bg-white rounded-xl border border-slate-200 p-4 flex items-center gap-3"
      >
        ...
        >
          <UserRound size={18} class="text-amber-600" />
        </div>
        <div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div class="space-y-2 max-h-[480px] overflow-y-auto pr-1">
            {#each stats.mentors as m}
              <button
                ...
                class="w-full flex items-center justify-between gap-2 p-2.5 rounded-lg border transition-colors {selectedMentorId ===
                m.id
              ...
              >
                <div class="flex items-center gap-2 min-w-0">
                  <div
                    class="w-7 h-7 rounded-full bg-slate-100 flex items-center justify-center shrink-0"
                  >
                ...
                <span
                  class="text-xs px-2 py-0.5 rounded-full shrink-0 {m.student_count >=
                  25
                    ? 'bg-red-100 text-red-600'
                    : m.student_count >= 20
                      ? 'bg-amber-100 text-amber-600'
                      : 'bg-emerald-100 text-emerald-600'}"
                  >{m.student_count}/25</span
                >
              </button>
            {/each}
          </div>
      ...
      <div class="lg:col-span-2 bg-white rounded-xl border border-slate-200 p-5">
        ...
        {:else}
          ...
          {:else}
            <div
              class="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-72 overflow-y-auto pr-1"
            >
              {#each filteredUnassigned as s}
                <button
                  onclick={() => toggleStudent(s.id)}
                  class="flex items-center gap-3 p-2.5 rounded-lg border text-left transition-colors {selectedStudentIds.has(
                    s.id,
                  )
                ...
                >
                  <div
                    class="w-5 h-5 rounded border shrink-0 {selectedStudentIds.has(
                      s.id,
                    )
                      ? 'bg-primary-600 border-primary-600'
                      : 'border-slate-300'} flex items-center justify-center"
                  >
                    {#if selectedStudentIds.has(s.id)}<span
                        class="text-white text-[11px] leading-none">✓</span
                      >{/if}
                  </div>
                  <div class="min-w-0">
    ...
    <div class="bg-white rounded-xl border border-slate-200 p-5">
      <div class="flex flex-wrap items-center justify-between gap-3 mb-3">
        <div>
          <h2 class="text-base font-semibold text-slate-900">
            {#if selectedMentor}{selectedMentor.name}'s Students{:else}All
              Assignments{/if}
          </h2>
          <p class="text-xs text-slate-500">
            {mentorAssignments.length} student(s)
          </p>
        </div>
          ...
          <tbody>
            {#each mentorAssignments as a}
              <tr class="border-b border-slate-100 hover:bg-slate-50/50">
                ...
                {#if !selectedMentorId}<td class="px-4 py-2 text-slate-500"
                    >{a.mentor_name}</td
                  >{/if}
                <td class="px-4 py-2">
                  {#if isAdmin}
            ...
            {:else}
              <tr
                ><td colspan="6" class="px-4"
                  ><EmptyState title="No assignments match" /></td
                ></tr
              >
            {/each}
          </tbody>
