<script lang="ts">
  ...
  async function loadGrid() {
    if (!cls || !selectedSubject) return;
    loading = true;
    statusMsg = "";
    const params = `class_id=${cls}&subject_id=${selectedSubject}&term=${term}`;
  ...
  $effect(() => {
    if (cls && selectedSubject) loadGrid();
  });
