...
candidates = [pathlib.Path("scripts/computer_awareness_bank.gift"), pathlib.Path(r"C:\Users\MDRS Bahaddurghatta\Downloads\question-bank-v3.gift")]
gift_path = None
for p in candidates:
    if p.exists():
        gift_path = p
...
pattern = re.compile(r"::(?P<title>[^:]+)::\s*(?P<qtext>.*?)\s*\{#(?P<marks>\d+)\}\s*(?P<meta>(?:\[.*?\]\s*)*)\{\s*(?P<opts>.*?)\s*\n\}", re.DOTALL)
def parse(m):
...
out = pathlib.Path("scripts/computer_awareness_seed.sql")
with open(out,"w",encoding="utf-8") as f:
    f.write("DO $$ DECLARE\n  sid UUID := '00000000-0000-0000-0000-000000000001';\n  caw_id UUID;\nBEGIN\n")
    f.write("  INSERT INTO subjects (id, name, code, school_id) SELECT gen_random_uuid(), 'Computer Awareness', 'CAW', sid WHERE NOT EXISTS (SELECT 1 FROM subjects WHERE code='CAW' AND school_id=sid AND deleted_at IS NULL);\n")
    f.write("  SELECT id INTO caw_id FROM subjects WHERE code='CAW' AND deleted_at IS NULL LIMIT 1;\n")
    f.write("  DELETE FROM quiz_responses WHERE question_id IN (SELECT id FROM questions WHERE subject_id=caw_id);\n")
    f.write("  DELETE FROM quiz_questions WHERE question_id IN (SELECT id FROM questions WHERE subject_id=caw_id);\n")
    f.write("  DELETE FROM questions WHERE subject_id=caw_id AND deleted_at IS NULL;\n")
    ...
    for q in qs:
        ...
        rows.append(f"  (sid, caw_id, 'mcq', '{esc(q['qtext'])}', '{esc(opts_json)}', '{q['ans_key']}', {q['marks']}, '{q['difficulty']}', '{esc(chapter_json)}', '{esc(tags_json)}', true)")
    f.write(",\n".join(rows)+";\nEND $$;\n")
