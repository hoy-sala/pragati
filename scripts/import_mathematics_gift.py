...
candidates = [pathlib.Path(r"C:\Users\MDRS Bahaddurghatta\Downloads\10th_std_mathematics_gift_questions.gift"), pathlib.Path("scripts/mathematics_bank.gift")]
gift_path = None
for p in candidates:
    if p.exists():
        gift_path = p
...
mat_id="9b3a16af-e2a1-4304-9f84-a3df549836ca"
c10_id="aca6ed6e-2263-4841-be61-10db6a280f21"
out = pathlib.Path("scripts/mathematics_150_seed.sql")
with open(out,"w",encoding="utf-8") as f:
    f.write("DO $$ DECLARE\n  sid UUID := '00000000-0000-0000-0000-000000000001';\n  mat_id UUID := '9b3a16af-e2a1-4304-9f84-a3df549836ca';\n  c10_id UUID := 'aca6ed6e-2263-4841-be61-10db6a280f21';\nBEGIN\n")
    f.write("  INSERT INTO class_subjects (class_id, subject_id) VALUES (c10_id, mat_id) ON CONFLICT DO NOTHING;\n")
    f.write("  DELETE FROM questions WHERE subject_id=mat_id AND deleted_at IS NULL;\n")
    ...
    for q in qs:
        ...
        rows.append(f"  (sid, mat_id, 'mcq', '{esc(q['qtext'])}', '{esc(opts_json)}', '{q['ans_key']}', {q['marks']}, '{q['difficulty']}', '{esc(chapter_json)}', '{esc(tags_json)}', true)")
    f.write(",\n".join(rows)+";\nEND $$;\n")
