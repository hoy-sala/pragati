...
# Try primary path (copied), fallback to original Downloads path
candidates = [pathlib.Path("scripts/social_science_bank.gift"), pathlib.Path(r"C:\Users\MDRS Bahaddurghatta\Downloads\10th_std_social_science_gift_questions.gift")]
gift_path = None
for p in candidates:
    if p.exists():
        gift_path = p
...
def norm_diff(d):
    d=d.strip().lower()
    if d in ("average","medium"): return "medium"
    if d in ("difficult","hard"): return "hard"
    if d in ("easy",): return "easy"
    return "medium"
def parse(m):
...
# quick sample
for q in qs[:2]:
    print(q["chapter"], q["qtext"][:60])
def esc(s): return s.replace("'", "''")
sid="00000000-0000-0000-0000-000000000001"
soc_id="e1069c35-9134-41fc-a3ff-b1e6c22bd4ef"
c10_id="aca6ed6e-2263-4841-be61-10db6a280f21"
out = pathlib.Path("scripts/social_science_320_seed.sql")
with open(out,"w",encoding="utf-8") as f:
    f.write("DO $$ DECLARE\n  sid UUID := '00000000-0000-0000-0000-000000000001';\n  soc_id UUID := 'e1069c35-9134-41fc-a3ff-b1e6c22bd4ef';\n  c10_id UUID := 'aca6ed6e-2263-4841-be61-10db6a280f21';\nBEGIN\n")
    f.write("  INSERT INTO class_subjects (class_id, subject_id) VALUES (c10_id, soc_id) ON CONFLICT DO NOTHING;\n")
    f.write("  DELETE FROM questions WHERE subject_id=soc_id AND deleted_at IS NULL;\n")
    ...
    for q in qs:
        ...
        rows.append(f"  (sid, soc_id, 'mcq', '{esc(q['qtext'])}', '{esc(opts_json)}', '{q['ans_key']}', {q['marks']}, '{q['difficulty']}', '{esc(chapter_json)}', '{esc(tags_json)}', true)")
    f.write(",\n".join(rows)+";\nEND $$;\n")
