...
func NewRouter(db *pgxpool.Pool, jwtService *auth.JWTService, cfg *config.Config) *chi.Mux {
	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.Route("/users", func(r chi.Router) {
			...
			r.Get("/teacher-assignments", roleMw.RequireRole("admin")(http.HandlerFunc(userH.TeacherAssignments)))
		r.Get("/me/teacher-detail", roleMw.RequireRole("admin", "principal", "teacher")(http.HandlerFunc(userH.MyTeacherDetail)))
